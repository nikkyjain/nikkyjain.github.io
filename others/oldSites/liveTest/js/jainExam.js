...
$(document).ready(function(){
    var myName='';
    if(getCookie('reviewName') != '') {
        myName=getCookie('reviewName');
        review=1;
        reviewName=myName;
    } else if (localStorage['jj__uName']) {
        myName=localStorage['jj__uName'];
    }
    if(myName != '') {
        $('#myName').val(myName);
        $('#myName').attr('readonly', true);
        $('#myName').css('border', '0');
        $('#myName').css('font-size', '2vw');
...
function postToServer() {
    if(localStorage['jj__uName']) {
        $.ajax({
            ...
            data : 'postType=exam&examType='+myStorageID+'&marks='+calcMarks()+'&'+$("#form").serialize(), // post data || get data
...
function restoreAns() {
    if(review) {
        // Get Answer from Server And Fill Form
        $.ajax({
            url: 'http://'+window.location.hostname+':8001/post', // url where to submit the request
            type : "POST", // type of action POST || GET
        data : 'postType=review&examType=margnaBandh&uName='+reviewName, // post data || get data
        success : function(result) {
            // you can see the result from the console
            // tab of the developer tools
...
function calcMarks(show) {
    ...
    if (show) {
        ...
        mySMDiv.innerHTML=marks+" / "+iCntr+" Marks ["+Math.round(marks*100/iCntr)+"%]";
    } else {
        console.log('Calculated Marks = '+marks);
        return (Math.round(marks*100/iCntr));
    }
}
...
function createMultipleChoice(ans, qDes, choice) {
    ...
    iCntr=qCntr;
    var idx="Que"+iCntr;
    document.write("<div class=d"+qNum%2+"><BR>"+qNum+") "+qDes+"<br><br>");
    document.write("<table cellspacing=10>");
    for(i=0;i<choice.length;i++) {
        if(i%2==0) { document.write("<tr>");}
        document.write("<td>&nbsp;&nbsp;<td><input type=radio name="+idx+" value=\""+(i+1)+"\"><label id="+idx+"_"+i+"> "+choice[i]+"</label></td>");
...
// Function to get a value of cookie
...
$(document).ready(function(){
    bodyFontSz=2;
    $('.incrExamFontSz').on('click', function(){
      console.log("Increasing Font Size"+$("body").css('font-size'));
      bodyFontSz+=.2;
      updateFontSz();
    })
    $('.decExamFontSz').on('click', function(){
      console.log("decreasing Font Size"+$("body").css('font-size'));
      bodyFontSz-=.2;
      updateFontSz();
    })
    function updateFontSz() {
        $("body").css({'font-size' : bodyFontSz+'vw'});
        $("label").css({'font-size' : bodyFontSz+'vw'});
        $("button").css({'font-size' : (bodyFontSz-1)+'vw'});
        $("select").css({'font-size' : (bodyFontSz-.2)+'vw'});
        $("select").css({'height' : (bodyFontSz+1)+'vw'});
        console.log("Updated Font Size"+$("body").css('font-size'));
    }
})
