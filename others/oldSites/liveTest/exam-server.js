...
app.post('/post', function(req, res){
    ...
    if(req.body.postType=='register') {
    ...
    } else if(req.body.postType=='login') {
        console.log("User "+req.body.loginUname+ ", Logging In");
        loginUser(req, res);
    } else if(req.body.postType=='result') {
        var finalResults=fResults+"/"+req.body.examType+".xml";
        console.log("Client Asking For Result" + finalResults);
        sendFileData(req, res, finalResults);
    } else if(req.body.postType=='delResult') {
        var finalResults=fResults+"/"+req.body.examType+".xml";
        console.log("Client Asking For Deleting Result Of " + req.body.uName + " From " + finalResults);
        delResult(req, res, finalResults);
    } else if(req.body.postType=='review') {
        console.log(req.body.uName+ " Asking For Review");
        var examDataFile=examData+"/"+req.body.examType+"/"+req.body.uName+".xml";
        sendFileData(req, res, examDataFile);
    } else if(req.body.postType=='exam') {
        console.log("Exam : "+req.body.examType+" -- Ans posted by "+req.body.mainUname+" : Marks "+req.body.marks);
        createDir(examData+"/"+req.body.examType);
        var examDataFile=examData+"/"+req.body.examType+"/"+req.body.mainUname+".xml";
        var finalResults=fResults+"/"+req.body.examType+".xml";
        if (!fs.existsSync(examDataFile)) {
            ...
            fs.writeFile(examDataFile, doc.toString(), function(err, data) {
                if(err) {
                    console.log("Error During XML Creation");
                    return console.log(err);
                }
                saveData(examDataFile, req, res);
                console.log("XML file created : "+examDataFile);
                addToFinalResults(finalResults, req, res);
            });
        } else {
            var ret={status: 'false', msg : "Re-Submission Of Answers Not Allowed. Contact Admin."};
            res.send(ret);
        }
    } else {
