...
sub aRow {
  my $self = shift;
  $self->{aRow}{keys %{$self->{aRow}}}=shift if (@_);
  return $self->{aRow};
}
sub answers {
  my $self = shift;
...
sub convert2html {
  ...
  $myaColumn=~s/, $/]/g;
  my $myaRow="[";
  for (my $i=0; $i<keys %{$self->{aRow}}; $i++) { $myaRow.="\"$self->{aRow}{$i}\", "; }
  $myaRow=~s/, $/]/g;
  my $myAnswers="[";
  ...
  print $FILE <<EOS
      ...
      var aRow=$myaRow;
      var ans=$myAnswers;
      createTableGridChose(\"$self->{description}\", qColumn, aColumn, aRow, ans);
EOS
}
